...
export default function UserMoreMenu({
  ...
  const [numero, setNumero] = useState(numeroClient);
  const [name, setName] = useState(nameClient);
  const [impot, setImpot] = useState(impotClient);
  ...
  const [email, setEmail] = useState(emailClient);
  ...
  const showSuccessToastSUP = () => {
    toast.warning('Client a été supprimé avec succès', {
  ...
  const modifyClient = () => {
    axios
      .put(
        `${process.env.REACT_APP_BASE_URL}/client/${idClient}`,
        {
          numero,
          name,
          impot,
