...
export default function UserMoreMenu({ idSite, indexSite, nameSite, sendInformation }) {
  ...
  const [index, setIndex] = useState(indexSite);
  const [name, setName] = useState(nameSite);
  ...
  const showSuccessToastSUP = () => {
    toast.warning('le site a été supprimé avec succès', {
  ...
  const modifySite = () => {
    axios
      .put(
        `${process.env.REACT_APP_BASE_URL}/site/${idSite}`,
        {
          index,
          name
        },
  ...
  return (
    <>
      ...
      >
        ...
        <MenuItem sx={{ color: 'red' }} onClick={() => deleteSite()}>
          <ListItemIcon>
