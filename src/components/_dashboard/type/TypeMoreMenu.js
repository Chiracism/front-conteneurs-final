...
export default function UserMoreMenu({ idType, indexType, nameType, sendInformation }) {
  ...
  const showSuccessToastSUP = () => {
    toast.warning('le type a été supprimé avec succès', {
  ...
  const showSuccessToastMOD = () => {
    toast.success('le type a été modifié avec succès', {
      position: toast.POSITION.TOP_RIGHT,
      autoClose: 1000
    });
  };
  ...
  const modifyType = () => {
    axios
      .put(
        `${process.env.REACT_APP_BASE_URL}/type/${idType}`,
        {
          index,
          name
        },
  ...
  return (
    <>
      ...
      >
        ...
        <MenuItem sx={{ color: 'red' }} onClick={() => deleteType()}>
          <ListItemIcon>
