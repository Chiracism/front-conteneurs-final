...
export default function UserMoreMenu({ idNavire, numeroNavire, nameNavire, sendInformation }) {
  ...
  const [numero, setNumero] = useState(numeroNavire);
  const [name, setName] = useState(nameNavire);
  ...
  const showSuccessToastSUP = () => {
    toast.warning('le navire a été supprimé avec succès', {
  ...
  const showSuccessToastMOD = () => {
    toast.success('le navire a été modifié avec succès', {
      position: toast.POSITION.TOP_RIGHT,
      autoClose: 1000
    });
  };
  ...
  const modifyNavire = () => {
    axios
      .put(
        `${process.env.REACT_APP_BASE_URL}/navire/${idNavire}`,
        {
          numero,
          name
        },
