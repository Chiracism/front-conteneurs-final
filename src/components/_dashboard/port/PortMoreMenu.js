...
export default function UserMoreMenu({ idPort, indexPort, namePort, sendInformation }) {
  ...
  const [index, setIndex] = useState(indexPort);
  ...
  const deletePort = () => {
    axios
      .delete(`${process.env.REACT_APP_BASE_URL}/port/${idPort}`, {
        headers: {
  ...
  const modifyPort = () => {
    axios
      .put(
        `${process.env.REACT_APP_BASE_URL}/port/${idPort}`,
        {
          index,
          name
        },
  ...
  return (
    <>
      ...
      >
        ...
        <MenuItem sx={{ color: 'red' }} onClick={() => deletePort()}>
          <ListItemIcon>
