...
export default function UserMoreMenu({ idOwner, numeroOwner, nameOwner, sendInformation }) {
  ...
  const [numero, setNumero] = useState(numeroOwner);
  const [name, setName] = useState(nameOwner);
  ...
  const showSuccessToastSUP = () => {
    toast.warning('le propriétaire a été supprimé avec succès', {
  ...
  const showSuccessToastMOD = () => {
    toast.success('le propriétaire a été modifié avec succès', {
      position: toast.POSITION.TOP_RIGHT,
      autoClose: 1000
    });
  };
  ...
  const modifyOwner = () => {
    axios
      .put(
        `${process.env.REACT_APP_BASE_URL}/Owner/${idOwner}`,
        {
          numero,
          name
        },
  ...
  return (
    <>
      ...
      >
        ...
        <MenuItem sx={{ color: 'red' }} onClick={() => deleteOwner()}>
          <ListItemIcon>
