...
export default function UserMoreMenu({ idRate, rateDateRate, amountRate, sendInformation }) {
  ...
  const [ratedate, setRateDate] = useState(rateDateRate);
  const [amount, setAmount] = useState(amountRate);
  ...
  const showSuccessToastSUP = () => {
    toast.warning('le taux a été supprimé avec succès', {
  ...
  const showSuccessToastMOD = () => {
    toast.success('le taux a été modifié avec succès', {
      position: toast.POSITION.TOP_RIGHT,
      autoClose: 1000
    });
  };
  ...
  const modifyRate = () => {
    axios
      .put(
        `${process.env.REACT_APP_BASE_URL}/Rate/${idRate}`,
        {
          ratedate,
          amount
        },
  ...
  return (
    <>
      ...
      >
        ...
        <MenuItem sx={{ color: 'red' }} onClick={() => deleteRate()}>
          <ListItemIcon>
